...
def assign(L, roads, students, buses, D, T):
    ...
    # Base case: after processing all students (index S)
    for state in range(state_size):
        bus_counts = decode_state(state, state_factors, B)
        total_assigned = sum(bus_counts)
        
        # Check if this is a valid final state
        if total_assigned == T:
            valid = True
            for i in range(B):
                if not (buses[i][1] <= bus_counts[i] <= buses[i][2]):
                    ...
                    break
            if valid:
                dp[S][state] = -2  # Valid terminal state
    
    # Fill DP table backwards from S-1 to 0
    for stud_index in range(S - 1, -1, -1):
        for state in range(state_size):
            if dp[stud_index + 1][state] is not None:
                # Option 1: Don't assign this student to any bus
                dp[stud_index][state] = -1
            
            # Option 2: Assign this student to a reachable bus
            current_bus_counts = decode_state(state, state_factors, B)
            total_assigned = sum(current_bus_counts)
            
            if total_assigned < T:  # Only assign if we haven't reached capacity
                for bus_index in student_bus_match[stud_index]:
                    if current_bus_counts[bus_index] < max_cap[bus_index]:
                        new_bus_counts = current_bus_counts.copy()
                        new_bus_counts[bus_index] += 1
                        new_state = encode_state(new_bus_counts, state_factors)
                        
                        if dp[stud_index + 1][new_state] is not None:
